...
#define IO_INTERRUPT_PORT_CNT (2u) // Only the Port 1 and Port 2 are interrupt capable.
...
static const struct io_config io_initial_configs[IO_PORT_CNT * IO_PIN_CNT_PER_PORT] = {
    ...
    // Output driver by timer A0, direction must be set to output
...
void io_get_current_config(io_e io, struct io_config *current_config)
{
    ...
    const uint8_t sel1 = (*port_sel1_regs[port]) ? 1 : 0;
    const uint8_t sel2 = (*port_sel2_regs[port]) ? 1 : 0;
    ...
    current_config->out = (*port_dir_regs[port] & pin) ? IO_OUT_HIGH : IO_OUT_LOW;
}
...
static void io_set_interrupt_trigger(io_e io, io_trigger_e trigger)
{
    ...
    io_clear_interrupt(io); // still needs to be cleared even if disabled
}
...
static void io_unregister_isr(io_e io)
{
    const uint8_t port = io_port(io);
    const uint8_t pin_idx = io_pin_idx(io);
...
void io_enable_interrupt(io_e io) { *port_interrupt_enable_regs[io_port(io)] |= io_pin_bit(io); }

void io_disable_interrupt(io_e io) { *port_interrupt_enable_regs[io_port(io)] &= ~io_pin_bit(io); }
...
static void io_isr(io_e io)
{
    ...
    io_clear_interrupt(io); // must explicitly clear interrupt in software
}

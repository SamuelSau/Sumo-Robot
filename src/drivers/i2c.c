...
static inline void i2c_set_tx_byte(uint8_t byte) { UCB0TXBUF = byte; }
...
static inline uint8_t i2c_get_rx_byte(void) { return UCB0RXBUF; }
...
{
    ASSERT(!initialized);
    static const struct io_config i2c_config = { .select = IO_SELECT_ALT3,
